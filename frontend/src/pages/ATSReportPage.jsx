...
import { getATSReport } from '../utils/api.js';
...
import VerdictBanner   from '../components/ats/VerdictBanner.jsx';
...
export default function ATSReportPage() {
  ...
  useEffect(() => {
    if (!reportId) return;
    (async () => {
      try   { const d = await getATSReport(reportId); setReport(d); }
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-xs font-black text-[var(--brand-primary)] uppercase tracking-widest">Generating Report...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-[Outfit,Inter,sans-serif] overflow-x-hidden pb-28">
      ...
      <div className="sticky top-0 z-50 flex justify-center pt-5 px-4 pointer-events-none">
        <nav className="w-full max-w-6xl pointer-events-auto floating-nav justify-between shadow-[0_8px_30px_rgba(0,0,0,0.06)] bg-white/95 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3">
            <span className="pill-highlight text-xs font-bold">✨ Analyze Resume</span>
            <button onClick={() => navigate('/ats')}
              className="text-xs py-2 px-5 rounded-full border border-black/10 hover:bg-zinc-50 transition-colors font-semibold text-slate-700">
              🔄 Check Another
            </button>
            <button onClick={() => window.print()}
              className="text-xs py-2 px-5 rounded-full bg-[#111827] hover:bg-black text-white font-semibold transition-colors shadow-sm">
              🖨️ Export PDF
            </button>
          </div>
        </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-10">
        <motion.div {...fade(0)}>
          <p className="text-[10px] font-black text-[var(--brand-primary)] uppercase tracking-[0.25em] mb-3">
            ✨ Analyze Resume
          </p>
      ...
      <div className="max-w-6xl mx-auto px-6 space-y-6">
        ...
        {/* Row 4 — Section Health + Formatting */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div {...fade(0.25)}>
            <SectionHealth sectionFeedback={report.sectionFeedback} />
          </motion.div>
      ...
      <div className="mt-20 text-center py-10 px-6 border-t border-black/[0.04]">
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em]">
          MockMate AI · ATS Resume Checker · Powered by Groq
        </p>
      </div>
    </div>
  );
...
/* ── Inline sub-components matching Dashboard card style ── */
...
function ScoreCard({ score = 0 }) {
  ...
  const { color, label, bg, pill } =
    clamp >= 85 ? { color: '#10B981', label: 'Strong Match',   bg: 'bg-emerald-50', pill: 'text-emerald-700 bg-emerald-50 border-emerald-200' } :
    clamp >= 70 ? { color: '#6B46C1', label: 'Good Match',     bg: 'bg-[var(--brand-light)]', pill: 'text-[var(--brand-primary)] bg-[var(--brand-light)] border-purple-200' } :
    clamp >= 50 ? { color: '#F59E0B', label: 'Moderate Match', bg: 'bg-amber-50', pill: 'text-amber-700 bg-amber-50 border-amber-200' } :
                  { color: '#EF4444', label: 'Weak Match',      bg: 'bg-red-50', pill: 'text-red-600 bg-red-50 border-red-200' };
  ...
  return (
    <div className="premium-card h-full flex flex-col items-center justify-center gap-5 py-10">
      ...
      <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border ${pill}`}>
        {label}
      </span>
    </div>
  );
...
function VerdictPanel({ verdict = '', verdictReason = '', strengthLines = [] }) {
  ...
  return (
    <div className="premium-card h-full flex flex-col gap-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-black uppercase tracking-wide border mb-2 ${cfg.pill}`}>
            {verdict}
          </span>
          ...
          <ul className="space-y-2">
            {strengthLines.map((line, i) => (
              <li key={i} className="flex gap-3 items-start">
                <span className="h-5 w-5 flex-shrink-0 mt-0.5 rounded-full bg-[var(--brand-light)] text-[var(--brand-primary)] flex items-center justify-center text-[10px] font-black">
                  {i + 1}
                </span>
